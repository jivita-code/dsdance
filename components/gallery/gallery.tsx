...
export function Gallery({ title, media }: { title: string; media: Media[] }) {
  ...
  return <>
    <div className="galleryGrid">{media.map((item, index) => <button className="galleryTile" data-reveal="card" data-reveal-delay={String(index % 4)} key={`${item.url}-${index}`} onClick={() => setActive(index)}>{item.type === "video" ? <video src={item.url} muted playsInline preload="metadata" aria-label={`${title} — video ${index + 1}`} /> : <img src={item.url} alt={`${title} — image ${index + 1}`} loading="lazy" />}<span>View {item.type}</span></button>)}</div>
